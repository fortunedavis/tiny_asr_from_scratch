...
class BestASR(nn.Module):
    def __init__(self, input_size=128, hidden=256, vocab_size=32):
        super().__init__()
        ...
        self.cnn_proj = nn.Linear(128, 256)
    ...
    def forward(self,x):
        ...
        feats =  self.relu(self.norm(feats))
...
def train_one_epoch(model, train_loader, valid_loader, loss_fn, id2label):
    model.train()
    total_loss = 0
    ...
    with torch.no_grad():
        ...
        logger.info(f"Valid Loss {100*valid_loss/len(valid_loader):.4f}")
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        train_one_epoch(model, train_loader, valid_loader, loss_fn, id2lab)
        scheduler.step()
